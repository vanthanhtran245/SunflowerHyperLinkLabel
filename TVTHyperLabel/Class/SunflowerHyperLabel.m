...
#import "SunflowerHyperLabel.h"
#import <CoreText/CoreText.h>

@interface SunflowerHyperLabel ()
...
@implementation SunflowerHyperLabel

static CGFloat highLightAnimationTime = 0.15;
static UIColor *SunflowerHyperLabelLinkColorDefault;
static UIColor *SunflowerHyperLabelLinkColorHighlight;

+ (void)initialize {
	if (self == [SunflowerHyperLabel class]) {
		SunflowerHyperLabelLinkColorDefault = [UIColor colorWithRed:28/255.0 green:135/255.0 blue:199/255.0 alpha:1];
		SunflowerHyperLabelLinkColorHighlight = [UIColor colorWithRed:242/255.0 green:183/255.0 blue:73/255.0 alpha:1];
	}
}
...
- (void)checkInitialization {
	...
	if (!self.linkAttributeDefault) {
		self.linkAttributeDefault = @{NSForegroundColorAttributeName: SunflowerHyperLabelLinkColorDefault,
									  NSUnderlineStyleAttributeName: @(NSUnderlineStyleSingle)};
	}
	
	if (!self.linkAttributeHighlight) {
		self.linkAttributeHighlight = @{NSForegroundColorAttributeName: SunflowerHyperLabelLinkColorHighlight,
										NSUnderlineStyleAttributeName: @(NSUnderlineStyleSingle)};
	}
}
...
- (void)setLinkForSubstring:(NSString *)substring withAttribute:(NSDictionary *)attribute andLinkHandler:(void(^)(SunflowerHyperLabel *label, NSString *substring))handler {
	...
	if (range.length) {
		[self setLinkForRange:range withAttributes:attribute andLinkHandler:^(SunflowerHyperLabel *label, NSRange range){
...
- (void)setLinksForSubstrings:(NSArray *)linkStrings withLinkHandler:(void(^)(SunflowerHyperLabel *label, NSString *substring))handler {
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	for (UITouch *touch in touches) {
		...
		if (rangeValue) {
			void(^handler)(SunflowerHyperLabel *label, NSRange selectedRange) = self.handlerDictionary[rangeValue];
